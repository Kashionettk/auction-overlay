// Plant Auction v5 – neu & sauber
// Quadratisch, Timer weiß→rot (Puls <15s), Top-3 mit goldenen Coins,
// Start-Button -> am Ende Winner + Feuerwerk + Reset-Button,
// TikFinity: window.updateDonator('{{username}}', {{diamondCount}})

(() => {
  // ===== KONFIGURATION =====
  let START_SECONDS = 180;  // 3:00 Startzeit
  let MIN_BID       = 200;  // nur Geschenke ab diesem Coin-Wert zählen
  let RESET_SECONDS = 15;   // bei Überbietung in letzten Sekunden -> auf 15s setzen
  ...
  function paint(){
    ...
    if (auctionRunning && totalSeconds <= Math.max(5, RESET_SECONDS)){
  ...
  function maybeLastSecondReset(){
    if (auctionRunning && totalSeconds <= Math.max(5, RESET_SECONDS)){
      totalSeconds = Math.max(5, RESET_SECONDS);
      paint();
    }
  }
  ...
  // ===== TikFinity-Schnittstelle =====
  // In TikFinity: Execute JavaScript auf deine Overlay-URL:
  // window.updateDonator('{{username}}', {{diamondCount}});
  window.updateDonator = function(name, amount){
    if (!auctionRunning) return;
    if (typeof amount !== 'number') amount = Number(amount) || 0;
    if (amount < MIN_BID) return;            // nur gültige Gebote zählen
